...
//getMixData
+ (id<TMapBeaconAnaUploader>) getMixDataUploader;
//getRealTimeEventData
+ (id<TMapBeaconAnaUploader>) getRealTimeEventDataUploader;


//注册一个混合数据到达上限的监听器，以便能接收信号
+ (BOOL) registMixDataUploadNotifier:(id<TMapAnaUplaodNotifier>) notifier;
//注册一个实时事件到达上限的监听器，以便能接收信号
+ (BOOL) registRealTimeEventDataUploadNotifier:(id<TMapAnaUplaodNotifier>) notifier;
